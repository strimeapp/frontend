...
function dragMarker(ev) {
	...
	var markerPosition = $("#video #video-inner #comments-layer .marker#"+markerID).position();
	...
	markerOriginalPositionTop = parseFloat( markerTop / parseInt( $("#video-inner #comments-layer").css("height") )) * 100;
	markerOriginalPositionLeft = parseFloat( markerLeft / parseInt( $("#video-inner #comments-layer").css("width") )) * 100;
    ...
    // Generate a click on this marker to activate it on the video and in the sidebar.
...
function dropMarker(ev) {
	...
	if((document.getElementById(markerID) !== null) && (document.getElementById(markerID).getAttribute('draggable') == 'true')) {
		...
		// Set the position of the marker in percentage
		finalMarkerPositionTop = parseFloat( mousePositionTop / parseInt( $("#video-inner #comments-layer").css("height") )) * 100;
		finalMarkerPositionLeft = parseFloat( mousePositionLeft / parseInt( $("#video-inner #comments-layer").css("width") )) * 100;

		// If the marker is out of the boundaries of the comment zone, reposition it
		if(finalMarkerPositionTop > 100) {
			finalMarkerPositionTop = parseFloat( parseInt( $("#video-inner #comments-layer").css("height")) -  parseInt( $("#video #video-inner #comments-layer .marker#"+markerID).css("height")) );
			finalMarkerPositionTop = parseFloat( finalMarkerPositionTop / parseInt( $("#video-inner #comments-layer").css("height") )) * 100;
		}
		else if(finalMarkerPositionTop < 0) {
			finalMarkerPositionTop = 0;
		}
		if(finalMarkerPositionLeft > 100) {
			finalMarkerPositionLeft = parseFloat( parseInt( $("#video-inner #comments-layer").css("width")) -  parseInt( $("#video #video-inner #comments-layer .marker#"+markerID).css("width")) );
			finalMarkerPositionLeft = parseFloat( finalMarkerPositionLeft / parseInt( $("#video-inner #comments-layer").css("width") )) * 100;
		}
		else if(finalMarkerPositionLeft < 0) {
			finalMarkerPositionLeft = 0;
		}
		$("#video #video-inner #comments-layer .marker#"+markerID).css("left", finalMarkerPositionLeft+'%').css("top", finalMarkerPositionTop+'%');
	...
	$.ajax({
		...
		success: function(data, textStatus, jqXHR){
			...
			if(obj.response_code != 200) {
				...
				$("#video #video-inner #comments-layer .marker#"+markerID).css("left", markerOriginalPositionLeft+'%').css("left", markerOriginalPositionTop+'%');
