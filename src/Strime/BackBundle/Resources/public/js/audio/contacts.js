$(document).ready(function() {
	...
	$(document).on("click", "#changeIdentityModal .contact", function() {
		...
		displayEmailAndAvatarInHeader(contactEmail, backAudioJsTextChangeId, userAvatar)
	...
	$("form#add-audio-contact-form").on("submit", function(e) {

		// Prevent the form to be submitted
		e.preventDefault();

		// Hide the previous message
		$("#add-audio-contact-result").empty().removeClass("alert-danger").removeClass("alert-succes").hide();

		// Set the variables
		var formdata = $('form#add-audio-contact-form').serialize();
		var ajaxAddAssetContactURL = $('form#add-audio-contact-form').attr("action");

		// Block the fields
		$('form#add-audio-contact-form input#form_email').attr("disabled", "disabled");

		$.ajax({
			type: 'POST',
			url: ajaxAddAssetContactURL,
			data: formdata,
			success: function(data, textStatus, jqXHR){
				...
				if(obj.response_code != 200) {
					...
					if(obj.error_source == "email_not_valid")
						var message = backAudioJsTextEmailAddressNotValid;

					else if(obj.error_source == "form_not_updated")
						var message = backAudioJsTextErrorSendingData;

					else if(obj.error_source == "contact_not_created")
						var message = backAudioJsTextErrorSavingEmailAddress;

					else
						var message = backAudioJsTextErrorOccuredDuringIdentification;

					// Display the message
					$("#add-audio-contact-result").empty().removeClass("alert-danger").removeClass("alert-succes");
					$("#add-audio-contact-result").text(message).addClass("alert-danger").fadeIn();

					// Unlock the fields
					$('form#add-audio-contact-form input#form_email').removeAttr("disabled");
				}

				// If the user has been created
				else {
					var message = backAudioJsTextThanksCanBeginCommenting;
					...
					// Display the message
					// $("#add-audio-contact-result").empty().removeClass("alert-danger").removeClass("alert-succes");
					// $("#add-audio-contact-result").text(message).addClass("alert-success").fadeIn();
					...
					setTimeout(function(){
						...
						// Empty the field
						$("form#add-audio-contact-form input#form_email").val("");
						...
						newContact += '</div>';
						newContact += '<div class="col-xs-2">';
						newContact += '<img src="'+iconCheck+'" title="'+backAudioJsTextChooseThisProfile+'" class="svg choose-identity">';
	...
	$(document).on("click", "#header-change-id #change-id", function() {
		...
		// Launch the modal
		$('body.audio .modal#changeIdentityModal button.close').show();
		$('#changeIdentityModal').modal({
	        'backdrop': true,
	        'keyboard': true,
	        'show': true
	...
	function displayEmailAndAvatarInHeader(contactEmail, backAudioJsTextChangeId, userAvatar) {
		var html = '';
		html += '<div class="contact-profile">';
		html += '<div class="contact-info">';
		html += '<div class="contact-email">'+contactEmail+'</div>';
		html += '<a href="#" id="change-id" title="' + backAudioJsTextChangeId + '">' + backAudioJsTextChangeId + '</a>';
		html += '</div>';
