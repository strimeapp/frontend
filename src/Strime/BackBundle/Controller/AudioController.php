...
{
    /**
     * @Route("/audio/{audio_id}/{action}/{url_data}", defaults={"action": NULL, "url_data": NULL}, name="app_audio")
     * @Template("StrimeBackBundle:App:audio.html.twig")
     */
    public function audioAction(Request $request, $audio_id, $action, $url_data)
    {
        ...
        // Set the endpoint
        $endpoint = $strime_api_url."audio/".$audio_id."/get";
        ...
        // Get the comments related to this audio
        // Set the endpoint
        $endpoint = $strime_api_url."audio/comments/".$audio_id."/get";
        ...
        if(($action != NULL) && (strcmp($action, "user") == 0) && ($url_data != NULL) && ($user_id == NULL)) {
            ...
            if($nb_comments > 0) {
                foreach ($comments as $comment) {
                    ...
                    $auto_login_token_check = hash('md5', $audio->{'user'}->{'user_id'}."-".$comment->{'comment_id'}."-".$this->container->getParameter('secret'));
        ...
        if($comments != NULL) {
            if($nb_comments > 0) {
                foreach ($comments as $comment) {
                    ...
                    $position = ($comment->{'time'} * 100) / $audio->{'duration'};
                    $comment->{'marker_left'} = $position;
                    ...
                    if(is_object($comment->{'author'})) {
                        ...
                        // Set the name
                        $comment->{'author'}->{'name'} = $this->get('translator')->trans('back.controller_app.audio.anonymous', array(), 'back_controller_app');
        ...
        if(($user_id != NULL) && (strcmp($user_id, $audio->{'user'}->{'user_id'}) == 0)) {
            $is_owner = TRUE;
        }
        else {
            $is_owner = FALSE;
        ...
        if($user_id == NULL) {
            ...
            // Get the contacts associated to the audio
            $contacts = array();
            foreach ($audio->{'contacts'} as $contact) {
            ...
            // Create the form the add a contact to the audio
            $add_audio_contact_form = $form->createAddAssetContactForm($audio_id, $audio->{'user'}->{'user_id'}, "audio");
        }
        ...
        // If the user is connected
            ...
            // Create the form the add a contact to the audio
            $add_audio_contact_form = NULL;
        }

        $twig_parameters = array(
            'body_classes' => 'audio asset',
        ...
        if($user_id != NULL) {
            ...
            $twig_parameters["add_audio_contact_form"] = $add_audio_contact_form->createView();
